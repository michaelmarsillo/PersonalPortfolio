      ...
      <div className="container mx-auto max-w-3xl px-2 sm:px-4">
            ...
            >
              {link.icon && (
                <>
                  ...
                  <span className="text-xs sm:text-base font-medium w-0 opacity-0 whitespace-nowrap overflow-hidden transition-all duration-500 ease-in-out group-hover:w-auto group-hover:opacity-100 group-hover:ml-1.5 sm:group-hover:ml-2">{link.name}</span>
                </>
              )}
              {!link.icon && <span className="text-xs sm:text-base font-medium theme-pill-hover">{link.name}</span>}
            </a>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-5 mt-3 sm:mt-4">
          <div className="flex justify-center items-center px-2">
              <div className="text-sm sm:text-[17px] font-semibold theme-accent-strong text-center">
                {new Date().getFullYear()} © Michael Marsillo
              </div>
          </div>
          <div className="flex items-center justify-center gap-2 sm:gap-4">
            
            <a href="https://wluring.xyz/api/michaelmarsillo/prev" className="text-xl sm:text-2xl theme-accent theme-accent-hover duration-500">←</a>
            ...
            <a href="https://wluring.xyz/api/michaelmarsillo/next" className="text-xl sm:text-2xl theme-accent theme-accent-hover duration-500">→</a>
          </div>
